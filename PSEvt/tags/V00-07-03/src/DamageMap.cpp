...
std::map<uint32_t,int> PSEvt::DamageMap::damageCounts() {
...
std::ostream & PSEvt::operator<<(std::ostream &o, PSEvt::DamageMap &damageMap) {
  PSEvt::DamageMap::iterator pos;
  bool first = true;
  o.setf(std::ios::hex,std::ios::basefield);
  for (pos = damageMap.begin(); pos != damageMap.end(); ++pos) {
    const PSEvt::EventKey &eventKey = pos->first;
    Pds::Damage & damage = pos->second;
    if (not first) o << ", ";
    first = false;
    o << eventKey << " damage=0x" << damage.value();
  }
  if (damageMap.splitEvent()) {
    std::vector<std::pair<Pds::Src,Pds::Damage> > droppedContribs = damageMap.getSrcDroppedContributions();
    o << "DroppedContributions: ";
    for (size_t idx=0; idx < droppedContribs.size(); ++idx) {
      std::pair<Pds::Src,Pds::Damage> & srcDamagePair = droppedContribs.at(idx);
      Pds::Src & src = srcDamagePair.first;
      Pds::Damage & damage = srcDamagePair.second;
