...
class GUIDark ( QtGui.QWidget ) :
    ...
    def __init__ ( self, parent=None ) :
        ...
        self.but_path = QtGui.QPushButton('Path:')
        self.but_check  = QtGui.QPushButton('Check status')
        ...
        self.grid.addWidget(self.but_check,  self.grid_row+1, 0, 1, 2)
        self.grid.addWidget(self.but_submit, self.grid_row+2, 1)
        self.grid.addWidget(self.but_status, self.grid_row+2, 2)
        self.grid.addWidget(self.but_wfiles, self.grid_row+2, 3)
        self.grid.addWidget(self.but_plot,   self.grid_row+2, 4)
        self.grid_row += 3

        self.connect(self.but_path,   QtCore.SIGNAL('clicked()'),     self.on_but_path )
        self.connect(self.but_check,  QtCore.SIGNAL('clicked()'),     self.on_but_check  )
    ...
    def setStyle(self):
        ...
        self.but_check  .setStyleSheet (cp.styleButton)
        ...
        #self.but_check  .setFixedWidth(width)
    ...
    def on_but_check(self):
        logger.debug('on_but_check - not implemented yet...', __name__)
        if bjpeds.status_for_pedestals() : self.but_check.setStyleSheet(cp.styleButtonGood)
        else                             : self.but_check.setStyleSheet(cp.styleButtonBad)
