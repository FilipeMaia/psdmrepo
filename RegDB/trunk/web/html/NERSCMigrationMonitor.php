...
<title>Monitor File Migration to NERSC</title>
...
try {
    ...
    print <<<HERE
<div id="header" >
  <span class="document_title">File Migration to NERSC:&nbsp;</span>
...
?>
    ...
    <table><tbody>
      <tr>
        ...
        <td class="table_hdr" colspan="4" align="center" >DAQ &Rarr; OFFLINE</td>
        <td class="table_hdr" colspan="4" align="center" >OFFLINE &Rarr; NERSC</td>
...
HERE;
        ...
        $daq2offline_status      = 'pending';
        $daq2offline_start_time  = null;
        $daq2offline_finish_time = null;

        if (array_key_exists($file_name, $files_daq2offline)) {
            $daq2offline_start_time = $files_daq2offline[$file_name]->start_time();
            if ($daq2offline_start_time) {
                $finish_time = $files_daq2offline[$file_name]->stop_time();
                if ($finish_time) {
                    if ($files_daq2offline[$file_name]->error_msg()) {
                        $daq2offline_status = 'failed';
                    } else {
                        $daq2offline_status = 'complete';
                        $daq2offline_finish_time = $finish_time;
                    }
                } else {
                    $daq2offline_status = 'in-progress';
                }
            }
        }
        $daq2offline_start_time_str  = is_null($daq2offline_start_time)  ? '&nbsp;' : $daq2offline_start_time->toStringShort();
        $daq2offline_finish_time_str = is_null($daq2offline_finish_time) ? '&nbsp;' : format_seconds($daq2offline_finish_time->sec - $daq2offline_start_time->sec);

        $offline2nersc_status      = 'pending';
        $offline2nersc_start_time  = null;
        $offline2nersc_finish_time = null;

        if (($daq2offline_status == 'complete') && array_key_exists($file_name, $files_offline2nersc)) {
            $offline2nersc_start_time = $files_offline2nersc[$file_name]->start_time();
            if ($offline2nersc_start_time) {
                $finish_time = $files_offline2nersc[$file_name]->stop_time();
                if ($finish_time) {
                    if ($files_offline2nersc[$file_name]->error_msg()) {
                        $offline2nersc_status = 'failed';
                    } else {
                        $offline2nersc_status = 'complete';
                        $offline2nersc_finish_time = $finish_time;
                    }
                } else {
                    $offline2nersc_status = 'in-progress';
                }
            }
        }
        $offline2nersc_start_time_str  = is_null($offline2nersc_start_time)  ? '&nbsp;' : $offline2nersc_start_time->toStringShort();
        $offline2nersc_finish_time_str = is_null($offline2nersc_finish_time) ? '&nbsp;' : format_seconds($offline2nersc_finish_time->sec - $offline2nersc_start_time->sec);
        ...
        $daq2offline_service_delay   = format_seconds($now->sec - $daq_open_time->sec);
        $offline2nersc_service_delay = format_seconds($now->sec - $daq_open_time->sec);

        $status = '';

        switch ($daq2offline_status) {
            case'complete':
                $daq2offline_service_delay  = format_seconds($daq2offline_finish_time->sec - $daq_open_time->sec);
                switch ($offline2nersc_status) {
                    case 'complete':
                        $offline2nersc_service_delay = format_seconds($offline2nersc_finish_time->sec - $daq_open_time->sec);
                        $status = '<span style="color:green;">complete</span>';
                        break;
                    case 'in-progress':
                        $status = '<span>migrating to NERSC...</span>';
                        break;
                    case 'pending':
                        $status = '<span style="color:blue;">waiting to migrate to NERSC...</span>';
                        break;
                    case 'failed':
                        $status = '<span style="color:red;">NERSC migration has failed</span>';
                        break;
                }
                break;
            case 'in-progress':
                $status = '<span>pulling from DAQ...</span>';
                break;
            case 'pending':
                $status = '<span style="color:blue;">waiting to migrate from DAQ...</span>';
                break;
            case 'failed':
                $status = '<span style="color:red;">migration from DAQ has failed</span>';
                break;
        }

        $size_str = '&nbsp;';
        $daq2offline_speed_mbps = '&nbsp;';
        $offline2nersc_speed_mbps = '&nbsp;';

        if ($size_bytes) {
            $size_str = $f->size < BYTES_IN_GB ? '<b>'.(intval($size_bytes / BYTES_IN_MB)).'</b> MB' : sprintf( "<b>%0.1f</b>", $f->size / BYTES_IN_GB).' GB';
            if ($daq2offline_status == 'complete') {
                $size_mb = intval($size_bytes / BYTES_IN_MB);
                $transfer_time_sec = $daq2offline_finish_time->sec - $daq2offline_start_time->sec;
                if ($transfer_time_sec > 0.)
                    $daq2offline_speed_mbps = sprintf("<b>%.1f</b> MB/s", $size_mb / $transfer_time_sec);
            }
            if ($offline2nersc_status == 'complete') {
                $size_mb = intval($size_bytes / BYTES_IN_MB);
                $transfer_time_sec = $offline2nersc_finish_time->sec - $offline2nersc_start_time->sec;
                if ($transfer_time_sec > 0.)
                    $offline2nersc_speed_mbps = sprintf("<b>%.1f</b> MB/s", $size_mb / $transfer_time_sec);
            }
        }
        $html =<<<HERE
        ...
        <td class="table_cell table_cell_highlight"               >{$daq2offline_start_time_str}</td>
        <td class="table_cell "                     align="right" >{$daq2offline_finish_time_str}</td>
        <td class="table_cell "                     align="right" >{$daq2offline_speed_mbps}</td>
        <td class="table_cell "                     align="right" >{$daq2offline_service_delay}</td>
        <td class="table_cell table_cell_highlight"               >{$offline2nersc_start_time_str}</td>
        <td class="table_cell "                     align="right" >{$offline2nersc_finish_time_str}</td>
        <td class="table_cell "                     align="right" >{$offline2nersc_speed_mbps}</td>
        <td class="table_cell "                     align="right" >{$offline2nersc_service_delay}</td>
