 ...
 ***********/
        ...
        if( this.data.length ) {
            ...
            for( var i in this.data ) {
                ...
                for( var j=0; j < row.length; ++j ) {
                    ...
                    html += '<td class="'+classes+'" '+selector+' align="'+this.header.align[j]+'">'+this.header.types[j].to_string(row[j])+'</td>';
                }
                html += '</tr>';
            }
        } else {
    ...
    column_types: function(coldef, types, sorted, selectable, align, next_column_number) {
        ...
        for( var i in coldef ) {
            var col = coldef[i];
            if( col.coldef ) {
                var child          = this.column_types(col.coldef, types, sorted, selectable, align, next_column_number);
                types              = child.types;
                sorted             = child.sorted;
