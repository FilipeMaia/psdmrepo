...
/**
 *  @defgroup CSPadPixCoords CSPadPixCoords package
 *  @brief Package CSPadPixCoords helps to access CSPAD pixel coordinates
 *  
 *  Detailed description.
 *  
 *  CSPAD is a detector composed of a set of tiles (AKA sensors, sectors, segments, 2x1, etc...).
 *  Sensor pixels' geometry is precisely defined by the silicon wafer production process. Relative 
 *  sensor relative position is not so well defined and may have a few pixel size offset from expected position.
 *  Geometry calibration parameters allows to get precise pixel location. 
 *  At data collection time some sensors may be turned on or off, 
 *  as it is defined in configuration parameters and affects the data array shape.
 *  
 *  CSPAD precise pixel geometry can be provided based on three components:
 *  @li description of the ideal sensor geometry 
