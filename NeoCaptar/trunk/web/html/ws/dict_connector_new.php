...
/**
 * This service will create a new connector type and return a dictionary of known
...
try {
    ...
    $connector_name          = NeoCaptarUtils::get_param_GET('connector_name');
    $connector_documentation = NeoCaptarUtils::get_param_GET('connector_documentation',true,true);
    $cable_name              = NeoCaptarUtils::get_param_GET('cable_name',false,false);
	...
	// Try finding or creating a new connector witin a separate transaction to avoid 
 	// a potential collision with other MySQL users who might be attempting to do
 	// the same in parallel. Note that in case of the detcted conflict 'add_dict_connector()'
    ...
    if( is_null( $connector )) {
        $connector = $neocaptar->add_dict_connector( $connector_name, $connector_documentation, $created_time, $created_uid  );
        if( is_null( $connector )) {
            $neocaptar->commit();
            $neocaptar->begin();
            $connector = $neocaptar->find_dict_connector_by_name( $connector_name );
    ...
    if( !is_null($cable_name)) {
        ...
        if( !$connector->is_linked($cable->id())) $connector->link($cable->id());
    }
