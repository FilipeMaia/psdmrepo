...
O2O_XTC_Repeat::O2O_XTC_Repeat ( const std::string& appName )
  : AppBase( appName )
  , m_count      ( parser(), "c,count",        "number",   "repeat count for L1Accept datagrams, def: 1", 1 )
  , m_dgramsize  ( parser(), "g,datagram-size","size",     "datagram buffer size. def: 16M", 16*1048576ULL )
  , m_minsize    ( parser(), "m,min-size",     "size",     "minimum datagram size to copy. def: 0", 0 )
  , m_outputDir  ( parser(), "d,output-dir",   "path",     "directory to store output files, def: .", "." )
  , m_outputName ( parser(), "n,output-name",  "template", "template string for output file names, def: .", "{seq4}.xtc" )
  , m_splitSize  ( parser(), "z,split-size",   "number",   "max. size of output files. def: 2G", 2*1073741824ULL )
