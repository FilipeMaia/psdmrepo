...
private:
  ...
  AppCmdOptGroup              m_inputOpt;
  AppCmdOptGroup              m_outputOpt;
...
O2O_Translate::O2O_Translate ( const std::string& appName )
  : AppBase( appName )
  , m_inputOpt(parser(), "Input options")
  , m_outputOpt(parser(), "Output options")
  , m_optionsFile( parser(),    "o,options-file",  "path",       "file name with options, multiple allowed", '\0' )
  , m_calibDir   ( m_inputOpt,  "C,calib-dir",     "path",       "directory with calibration data", "" )
  , m_compression( m_outputOpt, "c,compression",   "number",     "compression level, -1..9", -1 )
  , m_chunkSize  ( m_outputOpt, "k,chunk-size",    "size",       "HDF5 chunk size", 16*1024*1024 )
  , m_dgramsize  ( m_inputOpt,  "g,datagram-size", "size",       "option is ignored and deprecated", 0 )
  , m_dgramQSize ( m_inputOpt,  "Q,datagram-queue","number",     "datagram queue size", 32 )
  , m_experiment ( parser(),    "x,experiment",    "string",     "experiment name", "" )
  , m_extGroups  ( m_outputOpt, "G,group-time",                  "use extended group names with timestamps", false )
  , m_shortTimeStamp ( m_outputOpt, "short-timestamp",           "only store seconds and nanoseconds in time dataset", false )
  , m_noMissData ( m_outputOpt, "no-missing-data",               "do not fill missing data values", false )
  , m_noDamageDs ( m_outputOpt, "no-damage-ds",                  "do not store damage dataset", false )
  , m_instrument ( parser(), "i,instrument",       "string",     "instrument name", "" )
  , m_l1offset   ( m_inputOpt, "l1-offset",        "number",     "L1Accept time offset seconds", 0 )
  , m_mergeMode  ( m_inputOpt, "j,merge-mode",     "mode-name",  "merge mode", XtcInput::MergeFileName )
  , m_metadata   ( parser(), "m,metadata",         "name:value", "science metadata values", '\0' )
  , m_outputDir  ( m_outputOpt, "d,output-dir",    "path",       "directory to store output files", "." )
  , m_tmpDir     ( m_outputOpt, "D,tmp-dir",       "path",       "directory to write temporary HDF5 files", "" )
  , m_backupExt  ( m_outputOpt, "h5-backup-ext",   "string",     "extension to use for HDF5 backup", "" )
  , m_outputName ( m_outputOpt, "n,output-name",   "template",   "template string for output file names", "{seq4}.h5" )
  , m_overwrite  ( m_outputOpt, "overwrite",                     "overwrite output file", false )
  , m_runNumber  ( parser(), "r,run-number",       "number",     "run number, non-negative number", 0 )
  , m_runType    ( parser(), "t,run-type",         "string",     "run type, DATA or CALIB", "DATA" )
  , m_splitMode  ( m_outputOpt, "s,split-mode",    "mode-name",  "split mode", O2OHdf5Writer::NoSplit )
  , m_splitSize  ( m_outputOpt, "z,split-size",    "size",       "max. size of output files", 10*1073741824ULL )
  , m_liveDbConn ( m_inputOpt, "live-db",          "string",     "database connection string for live database", "" )
  , m_liveTable  ( m_inputOpt, "live-table",       "string",     "table name for live database", "file" )
  , m_liveTimeout( m_inputOpt, "live-timeout",     "number",     "timeout for live data in seconds", 120U )
