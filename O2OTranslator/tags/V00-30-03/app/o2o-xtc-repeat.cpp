...
O2O_XTC_Repeat::O2O_XTC_Repeat ( const std::string& appName )
  : AppBase( appName )
  , m_count      ( parser(), "c,count",        "number",   "repeat count for L1Accept datagrams", 1 )
  , m_dgramsize  ( parser(), "g,datagram-size","size",     "datagram buffer size", 16*1048576ULL )
  , m_minsize    ( parser(), "m,min-size",     "size",     "minimum datagram size to copy", 0 )
  , m_outputDir  ( parser(), "d,output-dir",   "path",     "directory to store output files", "." )
  , m_outputName ( parser(), "n,output-name",  "template", "template string for output file names", "{seq4}.xtc" )
  , m_splitSize  ( parser(), "z,split-size",   "number",   "max. size of output files", 2*1073741824ULL )
