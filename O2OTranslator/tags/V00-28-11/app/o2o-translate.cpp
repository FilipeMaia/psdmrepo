...
O2O_Translate::O2O_Translate ( const std::string& appName )
  ...
  , m_calibDir   ( m_inputOpt, "C,calib-dir",    "path",     "directory with calibration data, def: none", "" )
  , m_compression( m_outputOpt, "c,compression",  "number",   "compression level, -1..9, def: -1", -1 )
  , m_chunkSize  ( m_outputOpt, "k,chunk-size",   "size",     "HDF5 chunk size, def: 16M", 16*1024*1024 )
  , m_dgramsize  ( m_inputOpt, "g,datagram-size","size",     "option is ignored and deprecated", 0 )
  , m_dgramQSize ( m_inputOpt, "Q,datagram-queue","number",  "datagram queue size. def: 32", 32 )
  ...
  , m_l1offset   ( m_inputOpt, "l1-offset",      "number",   "L1Accept time offset seconds, def: 0", 0 )
  , m_mergeMode  ( m_inputOpt, "j,merge-mode",   "mode-name","one of one-stream, no-chunking, file-name; def: file-name",
                  XtcInput::MergeFileName )
  , m_metadata   ( parser(), "m,metadata",     "name:value", "science metadata values", '\0' )
  , m_outputDir  ( m_outputOpt, "d,output-dir",   "path",     "directory to store output files, def: .", "." )
  , m_tmpDir     ( m_outputOpt, "D,tmp-dir",      "path",     "directory to write temporary HDF5 files, def: ''", "" )
  , m_backupExt  ( m_outputOpt, "h5-backup-ext",  "string",   "extension to use for HDF5 backup, def: ''", "" )
  , m_outputName ( m_outputOpt, "n,output-name",  "template", "template string for output file names, def: {seq4}.h5", "{seq4}.h5" )
  ...
  , m_splitMode  ( m_outputOpt, "s,split-mode",   "mode-name","one of none, scan, or size; def: none", O2OHdf5Writer::NoSplit )
  , m_splitSize  ( m_outputOpt, "z,split-size",   "size",     "max. size of output files. def: 10G", 10*1073741824ULL )
  , m_liveDbConn ( m_inputOpt, "live-db",        "string",   "database connection string for live database", "" )
  , m_liveTable  ( m_inputOpt, "live-table",     "string",   "table name for live database, def: file", "file" )
  , m_liveTimeout( m_inputOpt, "live-timeout",   "number",   "timeout for live data in seconds, def: 120", 120U )
