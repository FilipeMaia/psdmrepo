...
from pyimgalgos.SegGeometry import *
...
#------------------------------
    ...
    def pixel_size_array(sp, axis) :
        """ Returns numpy array of pixel sizes in um for AXIS
        """
        return dict( zip( sp.AXIS, sp.get_pixel_size_arrs_um() ))[axis]


    def pixel_coord_array(sp, axis) :
        """ Returns numpy array of segment pixel coordinates in um for AXIS
        """
        return dict( zip( sp.AXIS, sp.get_cspad2x1_xyz_maps_um() ))[axis]


    def pixel_coord_min(sp, axis) :
        """ Returns minimal value in the array of segment pixel coordinates in um for AXIS
        """
        return dict( zip( sp.AXIS, sp.get_xyz_min_um() ))[axis]


    def pixel_coord_max(sp, axis) :
        """ Returns maximal value in the array of segment pixel coordinates in um for AXIS
        """
        return dict( zip( sp.AXIS, sp.get_xyz_max_um() ))[axis]
