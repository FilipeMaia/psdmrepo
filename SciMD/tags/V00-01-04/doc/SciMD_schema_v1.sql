...
-- -----------------------------------------------------
-- Table `SCIMD`.`EXPER_X_RUN_VAL_INT64`
-- -----------------------------------------------------
DROP TABLE IF EXISTS `SCIMD`.`EXPER_X_RUN_VAL_INT64` ;

CREATE  TABLE IF NOT EXISTS `SCIMD`.`EXPER_X_RUN_VAL_INT64` (
  `run_id` INT NOT NULL AUTO_INCREMENT ,
  `param_id` INT NOT NULL ,
  `val` BIGINT NULL ,
  PRIMARY KEY (`run_id`, `param_id`) ,
  INDEX `EXPER_X_RUN_VAL_INT64_FK_1` (`run_id` ASC) ,
  INDEX `EXPER_X_RUN_VAL_INT64_FK_2` (`param_id` ASC) ,
  CONSTRAINT `EXPER_X_RUN_VAL_INT64_FK_1`
    FOREIGN KEY (`run_id` )
    REFERENCES `SCIMD`.`RUN` (`id` )
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `EXPER_X_RUN_VAL_INT64_FK_2`
    FOREIGN KEY (`param_id` )
    REFERENCES `SCIMD`.`EXPER_X_RUN_PARAM` (`id` )
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
