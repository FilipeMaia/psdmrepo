...
function (
    ...
    var _SEREIS_NAME_FONT = _SEREIS_NAME_FONT_SIZE + 'pt Calibri' ;
        ...
        this.load = function (xRange, many_series) {
            ...
            for (var i in many_series) {
                var s = {
                    ...
                    color: _.isUndefined(many_series[i].color) ?
                        _DEFAULT_PLOT_COLOR[i % _DEFAULT_PLOT_COLOR.length] :
                        many_series[i].color
                } ;
...
//                    console.log('mouse not in any known region') ;
        ...
        this._initGeometry = function () {    
            ...
            for (var i = 0; i < this._series.length; ++i) {
                ...
                // Cache and use the widest width to avoid hjittering of
        ...
        this._drawAxisYofSeries = function (i, active) {
            ...
            if (active) {
                ...
                this._cxt.clearRect(xMin+1, yMin,   width-1, height) ;
//              this._cxt.fillStyle = '#fafafa' ;
//              this._cxt.fillRect(xMin+1, yMin,   width-1, height) ;
            }
        ...
        this._drawGridY = function () {
            ...
            for (var ystep = 0, y = this._geom.PLOT_Y_BEGIN[sid] ;      // no line at position 0
                     ystep < series.yLabels.get().length + 1 ;          // one extra line on top
        ...
        this._plotData = function (deltaX) {
            ...
            for (var sid = 0; sid < this._series.length; ++sid) {
                ...
                for (var j in series.points) {
                    ...
                    var x = x0 + this._geom.PLOT_WIDTH  * ((p[0] - this._xRange.min) / xDelta) ;
                    if (x < xmin) continue ;
                    if (x > xmax) continue ;

                    var y = this._geom.PLOT_Y_BEGIN[sid] - (this._geom.PLOT_Y_STEP_SIZE[sid] / series.yLabels.step) * (p[1] - series.yLabels.min) ;
