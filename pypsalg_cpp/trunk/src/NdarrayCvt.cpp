...
namespace psana_python {
  ...
  typedef ndarray<float,1> ndtype_1D;
  class myarray : public ndtype_1D 
  {
  public:
    myarray() : ndtype_1D() {
      std::cout << "****************************************" << std::endl;
      std::cout << "Created empty ndarray: " << this->data() << std::endl;
      std::cout << "refcount: " << m_data.use_count() << std::endl;
      std::cout << "****************************************" << std::endl;
    }
    
    myarray(ndtype_1D::shape_t* shape) : ndtype_1D(shape) {
      std::cout << "****************************************" << std::endl;
      std::cout << "created ndarray with shape: " << this->data() << std::endl;
      std::cout << "refcount: " << m_data.use_count() << std::endl;
      std::cout << "****************************************" << std::endl;
    }
    
    myarray(float* data, ndtype_1D::shape_t* shape) : ndtype_1D(data,shape) {
      std::cout << "****************************************" << std::endl;
      std::cout << "created ndarray with raw pointer to data: " 
		<< this->data()  << std::endl;
      std::cout << "refcount: " << m_data.use_count() << std::endl;
      std::cout << "****************************************" << std::endl;
    }
    myarray(boost::shared_ptr<float>& ptr, ndtype_1D::shape_t* shape) : 
      ndtype_1D(ptr,shape)
    {
      std::cout << "****************************************" << std::endl;
      std::cout << "created ndarray with BOOST shared pointer to data: "
		<< this->data() 
		<< std::endl;
      std::cout << "refcount: " << m_data.use_count() << std::endl;
      std::cout << "****************************************" << std::endl;
    ...
    virtual ~myarray() {
      std::cout << "****************************************" << std::endl;
      std::cout << "calling special myarray destructor: " 
		<< this->data()
		<< std::endl;    
      std::cout << "refcount: " << m_data.use_count() << std::endl; 
      std::cout << "****************************************" << std::endl;
  ...
  {  
    ...
    std::cout << "Calling converter from ndarray<"
	      <<  typeid( T ).name() << "," << Rank << "> to python object" 
	      << std::endl;
  ...
  void NDArrayToNumpy<T,Rank>::register_ndarray_to_numpy_cvt()  
  {
    ...
    if (reg == NULL) {
      std::cout << "REGISTER NDARRAY<" << typeid(T).name() << "," << Rank << ">"
		<< "TO NUMPY CONVERTER"
		<< std::endl;
      ...
      std::cout << "NDARRAY<" << typeid(T).name() << "," << Rank << ">"
  ...
  {
    std::cout << "BBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBB" << std::endl;
    std::cout << "CHECKING PYTHON OBJECT IS A NUMPY ARRAY" << std::endl;
    std::cout << "Value from PyArray_Check " << PyArray_Check(obj) 
	      << std::endl;
    if ( !PyArray_Check(obj) ) {
      std::cout << "PYTHON OBJECT IS NOT A NUMPY ARRAY" << std::endl;
      return NULL;
     }
    PyArrayObject* arrayPtr = reinterpret_cast<PyArrayObject*>(obj);
    const int rank = PyArray_NDIM(arrayPtr);
    if (rank != Rank) {
      std::cout << "NUMPY INCORRECT NUMBER OF DIMENSIONS" << std::endl;
      return NULL;
    }
    if (Traits<T>::numpyType() != PyArray_TYPE(arrayPtr)) {
      std::cout << "NUMPY INCORRECT TYPE" << std::endl;
      return NULL;
    }

    std::cout << "PYTHON OBJECT IS A NUMPY ARRAY" << std::endl;
    std::cout << "Leaving convertible" << std::endl;
    std::cout << "BBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBB" << std::endl;
    return obj;
  }
  ...
  void NumpyToNDArray<T,Rank>::construct(PyObject* obj, BoostData* boostData) 
  {
    ...
    std::cout << "Creating outgoing ndarray" << std::endl;
    ...
    std::cout << "Address of orignal Numpy data " << array << std::endl;
    std::cout << "BBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBB" << std::endl;
