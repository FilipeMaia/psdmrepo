...
ImgRadialCorrection::getAndProcImage(Event& evt)
{
  ...
  if (frmData.get()) {
    ...
    unsigned ind = 0;
      ...
      if (not data8.empty()) {
        if( m_print_bits & 8 ) MsgLog(name(), info, "getAndProcImage(...): Get image as ndarray<const uint8_t,2>");
	...
	for(cit=data8.begin(); cit!=data8.end(); cit++) { p_data[ind++] = double(int(*cit) - offset); }
      ...
      if (not data16.empty()) {
        if( m_print_bits & 8 ) MsgLog(name(), info, "getAndProcImage(...): Get image as ndarray<const uint16_t,2>");
