...
#-----------------------------
    ...
    def make_psana_cfg_file_for_peds_aver (self) :
        ...
        self.str_of_modules = ''
        self.txt_cfg_header = '# Autogenerated config file for psana, exp=%s, run=%s\n' \
                              % (cp.exp_name.value(), int(cp.str_run_number.value()))
        self.txt_cfg_body   = '\n'
...
#-----------------------------
    ...
    def cfg_file_body_for_peds_aver(self) :
        ...
        for det_name in cp.list_of_dets_selected() :
            ...
            for self.ind, (self.source, self.fname_ave, self.fname_rms) in enumerate(zip(lst, list_path_peds_ave, list_path_peds_rms)) :
                ...
                elif det_name == cp.list_of_dets[2] : self.print_warning()
                elif det_name == cp.list_of_dets[3] : self.add_cfg_module_peds_aver_princeton()
                elif det_name == cp.list_of_dets[4] : self.print_warning()
...
#-----------------------------
    ...
    def add_cfg_module_peds_aver_princeton (self) :
        self.path_in  = apputils.AppDataPath('CalibManager/scripts/psana-module-peds-aver-princeton.cfg').path()
        mod_img_rec = '%s:%i' % ('PrincetonImageProducer', self.ind) # str( cp.bat_img_rec_mod.value()
...
#-----------------------------
    ...
    def text_for_section (self) :
        ...
        """
        logger.info('Make text for: ' + self.path_in,__name__)
