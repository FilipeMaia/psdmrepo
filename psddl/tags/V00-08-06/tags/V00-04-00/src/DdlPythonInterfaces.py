...
class DdlPythonInterfaces ( object ) :
    ...
    def __init__ ( self, incname, cppname, backend_options ) :
        ...
        self.devtypes = backend_options.get('dev-types', "psddl_python/src/devicetypes.py")
        self.generics = {}
    ...
    def _parsePackage(self, pkgX):
        ...
        # open namespaces
        print >>self.py, ""
        print >>self.py, "%s = DeviceType()" % self.pkg.name
    ...
    def _createGetterClass(self, type):
        ...
        if generic != type.name and not generic in self.generics:
            self.generics[generic] = generic
            print >>self.py, '%s.%s = "Psana::%s::%s"' % (self.pkg.name, generic, self.pkg.name, generic)
