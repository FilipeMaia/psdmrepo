        ...
        if( !array_key_exists( $file->name, $result )) {
            $result[$file->name] =
                array (
                    ...
                    'storage'        => strtoupper($type) == 'XTC' ? 'RAW_DATA' : 'MEDIUM_TERM',
                	'type'           => $type,
                    'size'           => $file->size,
                    'created'        => $file->ctime,
...
$allowed_stay4storage = array (
    'RAW_DATA'    =>          SECONDS_IN_3_MONTHS,
    'MEDIUM_TERM' =>  1 * 4 * SECONDS_IN_3_MONTHS       // 1 year for the testing purposes
);
...
HERE;
    ...
    foreach( $run_numbers as $runnum ) {
        ...
        foreach( $files_by_runs[$runnum] as $file ) {
            ...
            if( !is_null($storage) && ( $storage != $file['storage'] )) continue;
            ...
            $allowed_stay_sec = $file['local_flag'] ? max( 0, $allowed_stay4storage[$file['storage']] - ( $now->sec - $file['created'] )) : 0;
            $allowed_stay     = $file['local_flag'] ? allowed_stay( $allowed_stay_sec ) : '';
        
            if( $file['local_flag'] && ( $allowed_stay_sec < 3600 )) {

                if( !array_key_exists( $file['storage'], $overstay_by_storage_run ))
                    $overstay_by_storage_run[$file['storage']] = array(
                        'total_runs'    => 0,
                        'total_files'   => 0,
                        'total_size_gb' => 0,
                        'runs'          => array()
                ...
                if( !array_key_exists( $runnum, $overstay_by_storage_run[$file['storage']] )) {
                    $overstay_by_storage_run[$file['storage']]['total_runs'] += 1;
                    $overstay_by_storage_run[$file['storage']]['runs'][$runnum] = array(
                        'files'   => 0,
                        'size_gb' => 0
                    );
                }
                $overstay_by_storage_run[$file['storage']]['total_files'  ]            += 1;
                $overstay_by_storage_run[$file['storage']]['total_size_gb']            += $size_gb;
                $overstay_by_storage_run[$file['storage']]['runs'][$runnum]['files'  ] += 1;
                $overstay_by_storage_run[$file['storage']]['runs'][$runnum]['size_gb'] += $size_gb;
            }
            $entry = array(
                'runnum'                 => $runnum,
                'filename'               => $filename,
            ...
            if(( $file['storage'] == 'RAW_DATA' ) && !$file['local_flag'] ) {
         ...
         */
        if( in_array( 'xtc', $types ) && ( is_null($storage) || ( $storage == 'RAW_DATA' ))) {
