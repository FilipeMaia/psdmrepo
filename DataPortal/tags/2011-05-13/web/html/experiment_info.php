...
try {
	...
	$experiment = isset( $exper_id ) ? $regdb->find_experiment_by_id         ( $exper_id   )
	                                 : $regdb->find_experiment_by_unique_name( $exper_name );
	if( is_null( $experiment )) {
		die( 'no such experiment found for '.( isset( $exper_id ) ? "id={$exper_id}" : "name={$exper_name}" ));
	}

	$leader_uid     = $experiment->leader_account();
	$leader_account = $regdb->find_user_account( $leader_uid );
