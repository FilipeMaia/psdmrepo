...
function elog_create() {
	...
	function post_allow_relevance_inputs( shift, runnum, datepicker, time ) {
		...
		obj = $('#el-p-shift'      ); if( shift      ) obj.removeAttr('disabled'); else obj.attr('disabled', 'disabled');
		obj = $('#el-p-runnum'     ); if( runnum     ) obj.removeAttr('disabled'); else obj.attr('disabled', 'disabled');
		obj = $('#el-p-datepicker' ); if( datepicker ) obj.removeAttr('disabled'); else obj.attr('disabled', 'disabled');
		obj = $('#el-p-time'       ); if( time       ) obj.removeAttr('disabled'); else obj.attr('disabled', 'disabled');
	}
	...
	this.post_init = function() {
		...
		$('#el-p-context-selector').buttonset();
		...
		$('#el-p-relevance-now').click(function(ev) {
			...
			if( context != 'shift' ) $('#el-p-shift' ).attr('disabled', 'disabled');
			if( context != 'run'   ) $('#el-p-runnum').attr('disabled', 'disabled');
		});
		...
		$('#el-p-relevance-shift').click(function(ev) {
			$('#el-p-datepicker').attr      ('disabled', 'disabled');
			$('#el-p-time'      ).attr      ('disabled', 'disabled');
			$('#el-p-shift'     ).removeAttr('disabled');
			$('#el-p-runnum'    ).attr      ('disabled', 'disabled');
		});
		$('#el-p-relevance-run').click(function(ev) {
			$('#el-p-datepicker').attr      ('disabled', 'disabled');
			$('#el-p-time'      ).attr      ('disabled', 'disabled');
			$('#el-p-runnum'    ).removeAttr('disabled');
		});
		...
		$('#elog-submit-and-stay').button().click(function() {
	
			/* Validate the form and initialize hidden fields */

			$('#elog-form-post input[name="author_account"]').val(that.author);
			$('#elog-form-post input[name="id"]').val(that.exp_id);
		...
		$('#elog-submit-and-follow').button().click(function() {
	...
	function post_reset() {
		...
		$('#el-p-context-experiment').attr      ('checked', 'checked');
		$('#el-p-context-shift'     ).removeAttr('checked');
		$('#el-p-context-run'       ).removeAttr('checked');
		$('#el-p-context-selector'  ).buttonset ('refresh');

		$('#el-p-relevance-shift').attr('disabled', 'disabled');
		$('#el-p-relevance-run'  ).attr('disabled', 'disabled');

		$('#el-p-relevance-now'     ).attr      ('checked', 'checked');
		$('#el-p-relevance-past'    ).removeAttr('checked');
		$('#el-p-relevance-shift'   ).removeAttr('checked');
		$('#el-p-relevance-run'     ).removeAttr('checked');
		$('#el-p-relevance-selector').buttonset ('refresh');
	...
	this.post_add_attachment = function(e) {
		if( e.value != '' ) {
			attachment_number++;
			$( '#el-p-as' ).append(
				'file: <input type="file" name="file2attach_'+attachment_number+'" onchange="elog.post_add_attachment(this)" />'+
				' '+
				'description: <input type="text" name="file2attach_'+attachment_number+'" value="" title="put an optional file description here" />'+
				'<br>'
			);
		}
	};
	...
	post_id2context['el-p-context-experiment']='experiment';
	post_id2context['el-p-context-shift']='shift';
	post_id2context['el-p-context-run']='run';

	function post_selected_context() {
		return post_id2context[$('#el-p-context-selector input:checked').attr('id')];
	}
	...
	this.live_init = function() {
		...
		$('#elog-live-range-selector').buttonset().change(function() {
			elog.live_reload();
		});
		$('#elog-live-runs-selector').buttonset().change(function() {
			elog.live_dim_day();
			elog.live_reload();
		});
		$('#elog-live-refresh-selector').buttonset().change(function() {
			if(live_selected_refresh()) {
				$('#elog-live-refresh-interval').removeAttr('disabled');
				that.live_schedule_refresh();
			} else {
				$('#elog-live-refresh-interval').attr('disabled', true);
				that.live_stop_refresh();
			}
		});
		$('#elog-live-expand').button().click(function() {
		...
		$('#elog-live-collapse').button().click(function() {
			that.live_dim_day();
			switch( that.live_expand_collapse ) {
			default:
			case 1:
		...
		$('#elog-live-viewattach').button().click(function() {
			that.live_dim_day();
		...
		$('#elog-live-hideattach').button().click(function() {
			that.live_dim_day();
		...
		$('#elog-live-refresh').button().click(function() {
			that.live_dim_all_highlights();
			that.live_refresh();
		});
		$('#elog-live-refresh-interval').change(function(ev) {
			that.live_stop_refresh();
			that.live_schedule_refresh();
		});
...
//		        }
		...
		if( $(container).hasClass('el-l-r-hdn')) {
			...
			$.get('../logbook/DisplayRunParams.php',{id:entry.run_id},function(data) {
				$('#el-l-r-con-'+entry.id).html(
					'<div style="width:820px; height:300px; overflow:auto; padding:10px; background-color:#ffffff; ">'+data+'</div>'
				);
				entry.loaded = true;
			});
		} else {
	...
	function live_refresh_interval() {
		return 1000*parseInt($('#elog-live-refresh-interval').val());
	}
	...
	this.live_update_info = function() {
		$('#el-l-ms-info').html(
			'<center>Loaded: <b>'+that.live_total_messages+'</b> messages'+
...
'          <div id="elog-live-reply-as-'+id+'" style="margin-top:5px;">'+
'            <div>'+
...
'      <textarea name="content" rows="12" cols="80" style="padding:4px;" title="the first line of the message body will be used as its subject" >'+entry.content+'</textarea>'+
'      <div style="font-weight:bold; margin-top: 10px;">Extra attachments:</div>'+
'      <div id="elog-live-edit-as-'+id+'" style="margin-top:5px;">'+
'        <div>'+
...
'</div>';
	...
	this.live_reply_add_attachment = function(id) {
		var num = $('#elog-live-reply-as-'+id+' > div').size();
		$('#elog-live-reply-as-'+id).append(
'  <div>'+
...
'  </div>'
	...
	this.live_edit_add_attachment = function(id) {
		var num = $('#elog-live-edit-as-'+id+' > div').size();
		$('#elog-live-edit-as-'+id).append(
'  <div>'+
...
'</div>';
	...
	this.live_highlight_day = function(idx) {
		...
		$('#el-l-ctx').removeClass('el-l-ctx-hdn').addClass('el-l-ctx-vis');
		$('#el-l-ctx-exp').html('<center>'+that.exp+'</center>');
		$('#el-l-ctx-day').html('<a class="link" href="#el-l-m-d-hdr-'+idx+'" title="go to the day header">'+day.ymd+'</a>');
		$('#el-l-ctx-info').html('<center><b>'+day.messages+'</b> messages'+( day.runs ? ', runs: <b>'+day.min_run+'</b> .. <b>'+day.max_run+'</b>' : '' )+'</center>');
	};

	this.live_dim_day = function() {
		if(that.live_current_day == null) return;
		that.live_current_day = null;
		$('#el-l-ctx').removeClass('el-l-ctx-vis').addClass('el-l-ctx-hdn');
	};
	...
	live_id2runs['elog-live-runs-on']=1;
	live_id2runs['elog-live-runs-off']=0;
	function live_selected_runs() {
		return live_id2runs[$('#elog-live-runs-selector input:checked').attr('id')];
	}

	var live_id2range = new Array();
	live_id2range['elog-live-range-20']    = 20;
	live_id2range['elog-live-range-100']   = 100;
	live_id2range['elog-live-range-shift'] = '12h';
	live_id2range['elog-live-range-day']   = '24h';
	live_id2range['elog-live-range-week']  = '7d';
	live_id2range['elog-live-range-all']   = null;
	function live_selected_range() {
		return live_id2range[$('#elog-live-range-selector input:checked').attr('id')];
	}

	var live_id2refresh = new Array();
	live_id2refresh['elog-live-refresh-on']=1;
	live_id2refresh['elog-live-refresh-off']=0;
	function live_selected_refresh() {
		return live_id2refresh[$('#elog-live-refresh-selector input:checked').attr('id')];
	}
