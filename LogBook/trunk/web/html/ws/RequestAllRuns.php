...
function report_error( $msg ) {
    ...
    print <<< HERE
{
  "Status": {$status_encoded}, "status": {$status_encoded},
  "Message": {$msg_encoded}, "message": {$msg_encoded}
}
HERE;
    exit;
}
    ...
    foreach( $experiment->runs() as $r ) {
        ...
        array_push(
            ...
            array(
                ...
                'durat'  => $durat,
                'durat1' => LogBookUtils::format_seconds_1( $r->end_time()->sec - $r->begin_time()->sec ),
                'sec'    => $total_seconds
            )
        );
    }
...
{
  "Status": {$status_encoded}, "status": {$status_encoded},
  "Updated": {$updated_encoded}, "updated": {$updated_encoded},
  "Runs": [
