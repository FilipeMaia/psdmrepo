...
ws_login_user     = 'logbook'  ## use -u option instead of this default
ws_login_password = ''         ## use -p option instead of this default
...
class LogBookGrabberUI:
    ...
    def __init__(self, parent):
        ...
        Label(parent, text="Experiment: ").grid(row=1,column=2,padx=5,pady=0,sticky=W+N)

        scrollbar = Scrollbar(parent, orient=VERTICAL)
        self.experiment = Listbox(parent, yscrollcommand=scrollbar.set,height=2)
        ...
        Label(parent, text="Author: ").grid(row=2,column=0,padx=5,pady=0,sticky=W+N)
        self.author = Label(parent, text=logbook_author)
        self.author.grid(row=2, column=1,padx=5,pady=0,sticky=W+N)

        Label(parent, text="Message: ").grid(row=3,column=0,padx=5,pady=5,sticky=W+N)
        self.message = ScrolledText(parent, width=48,height=4)
        self.message.grid(row=3,column=1,columnspan=2,rowspan=4,padx=5,pady=5,sticky=W+N)

        Label(parent, text="Description: ").grid(row=7,column=0,padx=5,pady=0,sticky=W+N)
        self.descr = Entry(parent)
        self.descr.insert(0, "Image #1")
        self.descr.grid(row=7, column=1,columnspan=2,padx=5,pady=0,sticky=W+N)

        self.canvas = Canvas(background='black',width=0,height=0)
        self.canvas.grid(row=8,column=0,padx=5,pady=5,columnspan=3,sticky=W+N+E+S)
...
def ws_get_experiments (experiment=None):
    ...
    try:
        ...
        # if the experiment was explicitly requested in the command line then make
        # sure the experiment is known. Otherwise return the whole list
        #
        if experiment is not None:
            for e in result['ResultSet']['Result']:
                if experiment == e['name']:
                    return { experiment : e }
            print "ERROR: no LogBook available for experiment: %s" % experiment
            sys.exit(1)
        else:
            d = dict()
            for e in result['ResultSet']['Result']:
...
OPTIONS & PARAMETERS:
  ...
  Optional parameters
    ...
    -e experiment
      ...
      the name of the experiment. If the name isn't provided
      the grabber will contac the LogBook Web service to request
...
""" % (progname)
    ...
    for name, value in opts:
        if name in ('-h',):
            ...
            print "password: "+ws_login_password
        else:
    ...
    logbook_experiments = ws_get_experiments (experiment)
