...
/*
 * This script will process a request for undeleting an existing free-form entry.
 */
...
try {
    ...
    $entry      = $logbook->find_entry_by_id( $id ) or report_error_and_exit( "no such message" );
    $experiment = $entry->parent();
    $instrument = $experiment->instrument();

    LogBookAuth::instance()->canPostNewMessages( $experiment->id()) or
        report_error_and_exit( 'You are not authorized to undelete messages of the experiment' );
