...
if( isset( $_GET['access'] )) {
    ...
    if( $access == '' ) {
        die( "access mode can't be empty" );
    }
}
if( !isset( $access )) $access = 'read';
...
try {
    ...
    // Leave only those experiments the logged user is authorizated to access in the specified
    // mode. If no mode is specified then assume 'read'.
    //
    
    function access_granted_for_experiment( $id ) {
        global $access;
        switch( $access ) {
        case 'read'         : return LogBookAuth::instance()->canRead           ( $id );
        case 'post'         : return LogBookAuth::instance()->canPostNewMessages( $id );
        case 'edit'         : return LogBookAuth::instance()->canEditMessages   ( $id );
        case 'delete'       : return LogBookAuth::instance()->canDeleteMessages ( $id );
        case 'manage_shifts': return LogBookAuth::instance()->canManageShifts   ( $id );
        }
        die( 'wrong access mode requested' );
    }
    $experiments = array();
    foreach( $all_experiments as $e ) {
        
        if( access_granted_for_experiment( $e->id())) {
