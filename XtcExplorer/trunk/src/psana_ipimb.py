...
class  psana_ipimb ( object ) :
    ...
    def __init__ ( self,
        ...
        print "psana_ipimb, %d sources: " % len(self.sources)
        for source in self.sources :
            print "  ", source

        self.quantities = opt.getOptStrings(quantities)
        print "psana_ipimb quantities to plot:"
    ...
    def event ( self, evt, env ) :
        ...
        for source in self.sources :
            ...
            # -------------------------------------------
            # fetch the data object from the psana event 
            # -------------------------------------------
            ...
            if ipm_raw is not None: 
                ...
                #print "psana_ipimb: no raw data from %s in event %d" % (source,self.n_shots)
            ...
            if ipm_fex is not None: 
                ...
                #print "psana_ipimb: no fex data from %s in event %d" % (source,self.n_shots)
