...
BEGIN
     ...
     SELECT COUNT(*) FROM files WHERE fk_fileset_id = in_fileset
         AND name = in_filename INTO fnrows;
...
BEGIN
     ...
     DECLARE fn_cur CURSOR FOR SELECT id FROM files AS fn
     ...
     IF (fsn_exists = 1) THEN
         ...
         */
         ...
         fnl: LOOP
             ...
             UPDATE files SET fk_fileset_status = in_status 
             WHERE files.id = lcl_id; 
         END LOOP fnl;
...
END//
SHOW WARNINGS//
DROP procedure IF EXISTS `new_fileset_nocache` //
SHOW WARNINGS//
/*
** Create a new fileset without trying to
** find cache space for it.
*/
CREATE PROCEDURE `interface_db`.`new_fileset_nocache` (
                IN in_experiment TINYTEXT, 
                IN in_instrument TINYTEXT,
                IN in_run_type TINYTEXT,
                IN in_run_number INT UNSIGNED,
...
END//
SHOW WARNINGS//
DROP procedure IF EXISTS `add_file_nocache` //
SHOW WARNINGS//
/*
** Add file to existing fileset without size information
** for the no cache option. Out_status returns NULL if not success.
*/
CREATE PROCEDURE `interface_db`.`add_file_nocache` (
             IN in_fileset INT UNSIGNED, 
             IN in_filetype ENUM('XTC','EPICS'),
             IN in_filename TEXT, 
             OUT out_status TINYINT)
BEGIN
     ...
     IF fileset_exists AND fnrows = 0 THEN
         /*
         ** Insert new row into files.
         */
         SELECT id FROM fileset_status_def AS fsd
             WHERE fsd.name = 'Initial_Entry' INTO lcl_filename_status_id;
         INSERT INTO files
