...
_defOutputDir = "/reg/d/psdm/%(instrument_lower)s/%(experiment)s/mon/data-summary"
...
class TranslatorJob(object) :
    ...
    def __build_translate_cmd(self, fs, outputDir, outputDirTmp ) :
        ...
        # output dir
        cmd.extend(("--base-output-dir", outputDir))
        
        events_to_process = self._get_config('nevents', 100)
        numproc = self._get_config('lsf-numproc', 1)
        if numproc > 0:
            events_per_node = int(math.ceil(float(events_to_process)/float(numproc)))
        else:
            events_per_node = events_to_process
        cmd.extend(("--max-events-per-node", "%s" % events_to_process))

        cmd.append("%s/%s" % (fs.instrument, fs.experiment))
        cmd.append("%d" % fs.run_number)
