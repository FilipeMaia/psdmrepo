...
namespace {
  ...
  PyObject* pixel( PyObject* self, PyObject* args );
...
next( PyObject* self, PyObject* args )
{
  ...
  // get Pds::CsPad::ConfigV1 from argument
  if ( not pypdsdata::CsPad::ConfigV1::Object_TypeCheck( configObj ) ) {
    PyErr_SetString(PyExc_TypeError, "Error: parameter is not a cspad.ConfigV1 object");
    return 0;
  }
  const Pds::CsPad::ConfigV1* config = pypdsdata::CsPad::ConfigV1::pdsObject( configObj );
  Pds::CsPad::ElementV1* next = const_cast<Pds::CsPad::ElementV1*>(obj->next( *config ));
  unsigned payloadSize = config->payloadSize();
...
data( PyObject* self, PyObject* args )
{
  ...
  unsigned nAsics = config->numAsicsRead();
  ...
  // dimensions
  npy_intp dims[4] = { nAsics/2, Pds::CsPad::ColumnsPerASIC, Pds::CsPad::MaxRowsPerASIC, 2 };

  // make array
  PyObject* array = PyArray_New(&PyArray_Type, 4, dims, typenum, 0,
...
PyObject*
pixel( PyObject* self, PyObject* args )
{
  const Pds::CsPad::ElementV1* obj = pypdsdata::CsPad::ElementV1::pdsObject( self );
  if ( not obj ) return 0;

  // parse args
  unsigned asic, col, row ;
  if ( not PyArg_ParseTuple( args, "III:ElementV1.pixel", &asic, &col, &row ) ) return 0;
  
  return PyInt_FromLong( *obj->pixel(asic, col, row) );
}
