  ...
  , _timeStampLo(0)
{
  ...
  ndarray<uint16_t, 2>::const_iterator src0 = frames.frame(0).data().begin();
  ndarray<uint16_t, 2>::const_iterator src3 = frames.frame(3).data().begin();
  for (int iY = 0; iY < 512; ++ iY, src0 += 512, src3 += 512) {
    dest = std::copy(src0, src0+512, dest);
    dest = std::copy(src3, src3+512, dest);
  }

  ndarray<uint16_t, 2>::const_reverse_iterator src1 = frames.frame(1).data().rbegin();
  ndarray<uint16_t, 2>::const_reverse_iterator src2 = frames.frame(2).data().rbegin();
